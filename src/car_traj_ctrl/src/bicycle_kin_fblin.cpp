...
void bicycle_kin_fblin::reference_transformation_position(double xref, double yref, double &xPref, double &yPref)
{
    xPref = xref + P_dist*std::cos(theta);
    yPref = yref + P_dist*std::sin(theta);
}

void bicycle_kin_fblin::reference_transformation_velocity(double dxref, double dyref, double dthref, double &dxPref, double &dyPref)
{
    dxPref = dxref - P_dist*dthref*std::sin(theta);
    dyPref = dyref + P_dist*dthref*std::cos(theta);
}