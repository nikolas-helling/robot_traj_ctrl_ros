...
void bicycle_sim::Prepare(void)
{
    ...
    /* ROS topics */
    vehicleCommand_subscriber = Handle.subscribe("/car_input", 1, &bicycle_sim::vehicleCommand_MessageCallback, this);
...
void bicycle_sim::RunPeriodically(void)
{
...
void bicycle_sim::Shutdown(void)
{
    // Delete ode object
    delete simulator;
...
void bicycle_sim::vehicleCommand_MessageCallback(const std_msgs::Float64MultiArray::ConstPtr& msg)
{
...
void bicycle_sim::PeriodicTask(void)
{
    /*  Integrate the model */
    simulator->integrate();
